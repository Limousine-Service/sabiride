...
class Appsettings_model extends CI_model
{
    ...
    public function getSettingCoin() {
        $this->db->select('setting_coin');
        $this->db->from('app_settings')->limit(1);

        return $this->db->get()->result();

    }

    public function updateSettingCoin($coin) {
        return $this->db->update("app_settings", ["setting_coin" => $coin]);
    }
